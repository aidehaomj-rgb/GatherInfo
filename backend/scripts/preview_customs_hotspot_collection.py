...
def _payload(item) -> dict:
    ...
    return {
        ...
        "china_customs_stage": review.get("china_customs_stage") or "待核验",
        "transmission_chain": review.get("transmission_chain") or "待核验",
        "china_customs_score": review.get("china_customs_score") or 0,
        "transmission_evidence_score": review.get("transmission_evidence_score") or 0,
        "executable_check_score": review.get("executable_check_score") or 0,
...
def _markdown(payload: dict) -> str:
    ...
    for index, item in enumerate(payload["items"], 1):
        lines.extend([
            ...
            f"- 中国海关监管环节：{item['china_customs_stage']}",
            f"- 传导链条：{item['transmission_chain']}",
            f"- 中国海关价值评分：{item['china_customs_score']}",
            f"- 传导依据评分：{item['transmission_evidence_score']}",
            f"- 核查可执行性评分：{item['executable_check_score']}",
