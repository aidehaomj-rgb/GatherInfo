...
## 2026-08-05 可复用采集提示词模块

1. `PromptTemplate` 独立维护提示词名称、说明、正文与启停状态；主题通过 `prompt_template_ids` 挂载零到多份提示词。
2. 采集时按主题配置顺序合并临时采集说明、主题 `description_prompt` 与所有已启用的挂载提示词，并统一用于语义检索规划及候选处理；停用提示词不参与采集。
3. 删除提示词时必须同步从所有主题解绑，禁止保留悬空 ID；内置提示词只在缺失时初始化，不覆盖用户后续编辑。
4. 前端“提示词库”负责 CRUD，主题配置弹窗负责挂载；相关变更至少执行提示词后端测试和前端生产构建。

---

## 2026-08-03 5N Plus两用关键材料供应链

1. 美国方向新增 `inv-us-5n-plus-dual-use-materials`，链路覆盖5N Plus上虞生产、香港贸易协调、
   加拿大集团整合，以及美国First Solar、布里奇波特铋材料、圣乔治半导体和SDA红外项目。
2. `backend/scripts/import_5n_plus_dual_use_supply_chain.py` 为幂等导入脚本；贸易数据按报告原始口径
   保存为3个汇总组、7批、120,934千克，不虚构报告未披露的逐票日期和提单号。
3. 集团关系、政府投资、光伏供应协议和SDA项目可标记为已核验；中国来源碲/硒具体进入First Solar
...
## 2026-08-05 印度、日本供应链深检与对台军售筛查

1. `backend/scripts/enrich_india_japan_evidence_20260805.py` 为幂等补充脚本，覆盖印度9条、
   日本4条现有调查；每条调查都新增官方/企业/媒体证据和独立的台湾军售筛查记录。
2. 日本企业主体必须分开：当前三菱防卫航空发动机业务属于三菱重工本体，IHI株式会社负责
   F7-10等航空发动机而IHI Aerospace另承接宇航项目，NEC本体与NEC Aerospace Systems
   也不得混用。NEC向印度海军转移UNICORN已由日本防卫省确认，约15亿日元是规格调整计划
   认定额，不是装备销售合同总价。
3. 对台方向仅确认一项历史直接装备转移：三菱造船下关造船所在1950年代向台湾海军转移2艘
   “复仇”级鱼雷艇。该主体为现三菱重工的历史合并前主体，必须标注为历史案例，不得延伸为
   当前航空发动机关键材料链对台销售。
4. Dhaksha公开涉台信息是企业声称飞控替代件来自美国和台湾，方向为台湾零部件进入印度，
   不是印度对台军售；台船泵组虽进入台湾企业，但官方EPC文件和货描均闭环至台湾中油大林
   石化项目，状态保持 `verified_non_defense`。
5. Dhanush的6只中国制造轴承已由印度政府文件闭环至炮车工厂和火炮项目；Dhaksha中国来源
   测试/校准设备、NewSpace以美替代件及其余日本材料链仍缺BOM、料号或逐批终端记录，必须
   保持 `follow_up`。变更后需复跑脚本验证幂等性，执行SQLite外键/完整性检查、调查接口核对
   和前端生产构建。
## 2026-08-06 穆格—天和稀土永磁体涉台供应链

1. `backend/scripts/import_taiwan_moog_tianhe_pac3_chain.py` 将用户报告作为 `inv-taiwan-lead-moog-tianhe-pac3-magnets` 导入台湾方向，贸易记录只保存报告汇总（至少8票、约26.5吨、7票中国直发/1票韩国中转），不虚构逐票提单、HS编码或法律实体。
2. Moog官方PAC-3 MSE作动器公告、美国国会/DSCA台湾PAC-3 MSE记录分别作为独立证据；后者记录100枚PAC-3 MSE及2枚测试弹。两者不能自动证明报告磁体批次已进入台湾军售装备。
3. 只要缺少元素含量、BOM、批号/序列号及最终用户文件，磁体到PAC-3/Black Arrow的关系保持 `C/follow_up`；报告中的2026年“102枚”不作为独立核验事实。
4. 台湾方向主图中的中国供应端必须同时展示天和磁材与成都银河磁体两个并列节点，二者均只指向穆格，再由穆格指向洛克希德·马丁、最后指向台湾；成都银河磁体法定全称保留为“成都银河磁体股份有限公司”，并兼容“成都银河磁体有限公司”等检索别名。

---

## 2026-08-08 Rossell—AH-64涉台线索与IHI F110排歧

1. `backend/scripts/import_india_rossell_ah64_taiwan_lead.py` 幂等导入并排歧中国绍兴Kabletex—印度Rossell线索；贸易记录保存2025-12-12三组提单号和681系列线缆零件号，不把商业数据库的无单位“Amount”误写为数量或金额。
2. 681-090076同基号多个后缀由二级市场索引为Lam Research蚀刻设备零件；NBD披露681-096871同基号另一后缀为UL3271线，UL官方将该样式定义为电机引线/电器内部布线；Rossell官方投资者演示又明确列有Lam Research半导体设备样机与量产业务。精确后缀仍未全部映射，但当前证据明显反对AH-64归因。
3. 中国线缆到AH-64及台湾机队的两段关系必须保持`C/rejected`和`is_reportable=false`；图谱不得绘制跨层连线。台湾30架阿帕奇原始军售早于Rossell公开的2017年AH-64合同，后续支持任务单的16条一级分包记录也未出现Rossell。
4. IHI的F110许可生产官方用途为日本F-2；台湾F-16 Block 70案虽含75台GE F110，但型号相同不等于供应链相同。没有序列号、分包清单或IHI出口文件前，不构成日本对台军售链。
5. `backend/scripts/enrich_japan_export_control_boundaries_20260808.py` 按商务部公告逐一限定被列名法律主体：川崎重工航空宇宙系统公司可直接对应，三菱、IHI、NEC只能对应公告列出的具体子公司，不得扩大到集团本体。
6. 日本政府确认自卫队保有PAC-3转交美国，并要求防止未经日本事先同意的第三方转移；该事实不披露制造商、批次或台湾去向，只能作为涉台筛查边界，不能与美国对台PAC-3军售自动拼接。
7. `SupplyChainOpenSourceEvidence.case_id` 为空时表示调查级证据；指定 `investigation_id` 的接口必须按调查已挂载ID返回，不能因案件内连接而静默漏掉。未指定调查时仍按案件国家过滤。

---

## 2026-08-08 系统工作台与供应链穿透视觉优化

1. 全局工作台顶部显示当前功能、数据链路状态和当日采集量；侧栏固定为可滚动导航，较矮屏幕下仍须能访问全部功能入口。
2. 页面切换、侧栏扫描与状态脉冲使用轻量动态效果，所有动画必须在 `prefers-reduced-motion` 下停用或降级。
3. 供应链穿透统一使用深色情报工作台视觉；不得再混用浅色完整度面板、证据标签或图谱区域。
4. 动态供应链图谱必须完整展示全部已关联主体和项目，不得仅截取前三项；长名称和关系说明允许自然换行，并通过增大节点间距避免重叠。
5. 图谱提供节点聚焦阅读、动态效果暂停、节点/关系数量以及“穿透链路清单”；清单与关键发现不得对核验缺口或报告数量做静默截断。
6. 供应链图谱颜色语义保持一致：红色表示上游贸易与供应、青色表示核心整合、琥珀色表示下游承接、绿色表示最终应用。
7. 本轮前端验证基线：`npm --prefix frontend run typecheck` 与 `npm --prefix frontend run build`。
8. “系统配置 → 界面与显示”管理设备级界面偏好：全局字号为90%—120%、内容密度为紧凑/舒适/宽松，并支持减少动态效果和增强文字对比度。
9. 界面偏好保存在浏览器本地存储，通过 `utils/uiPreferences.ts` 统一校验、应用和恢复默认；应用启动渲染前必须先恢复偏好，避免页面闪动。
10. 界面偏好不写入后端业务配置，也不随配置导入导出传播；本地存储不可用时仍应保证当前会话能够应用设置。

---

## 2026-08-16 供应链专家通用采集逻辑

1. 供应链专家默认执行三轮联网研究：首轮建立中国来源与军事用途信号集，后续轮次按企业、别名、地址、产品/料号、合同号、提单号和证据缺口生成精确补证查询。
2. 贸易证据必须验证“中国出口商 → 目标国家进口企业”的方向，并符合用户选择的近一季度、近半年或近一年窗口；合同证据可早于贸易记录。
3. 公开贸易表格采用确定性字段解析，模型超时或漏选时仍保留合格的贸易单边线索；合同单边与贸易单边按企业和产品语义合并，未闭合部分明确写入待补证内容。
4. 公开贸易索引探测由本轮识别出的进口企业动态生成，不维护单个样本企业的固定 URL；国家方向企业种子只作为实体抽取失败时的有界兜底。
5. 新发现写入前同时检查既有供应链和待审核候选；同一候选再次取得新证据时更新原记录，不重复创建。
6. 本轮验证基线：供应链、连接器、API、提示词和标签相关后端测试共 99 项通过；`npm --prefix frontend run build` 通过。

---

## 2026-08-17 六主题两轮采集闭环与 InfoRoute 采集手册融合

1. 仅“中国出口管制、关税类贸易政策、外贸论坛违规清关与走私风险线索、执法信息采集、
   技术性贸易措施、涉进出口时政热点”进入最多两轮的 `CollectionBatch` 闭环；美国军工采购与
   供应链、关键矿产开源信息情报保持原流程。
2. `TopicResearchContext` 是 planner、质量审核和验收的统一不可变上下文，必须贯穿同义词、
   排除词、重点国家/语言、目标网址、挂载提示词、时间窗和主题默认 policy；connector 不得设置
   通用免过滤标记。
3. 动态周目标按最近8个完整北京自然周 P75×1.2 并限制在主题范围；第一轮广覆盖，第二轮只补
   地域、政策工具、来源集中和证据等级缺口。达到目标、无新 URL 或两轮用尽必须停止并持久化原因。
4. 正式信息必须有稳定原文 URL、页面可核验日期、独立正文、0—1 评分、主题审核与证据/中国关联
   等级；排除词始终生效。候选只在任务内存中存在，不建持久化候选池。
5. InfoRoute `collection_playbook.md` 的采集纪律已纳入：来源许可未知即不自动全文处理；优先
   API/RSS/sitemap/栏目入口；跟踪参数入库前剥离；不从正文猜发布日期；每条保存原始来源、发现
   入口、locator、发布/更新时间、正文哈希和抓取时间。连续3次来源失败进入运行时
   `paused_drift_review`，不得永久改写用户启停配置。
6. 跨主题复用统一写 `ItemTopicMembership`，条目、统计、标签、报告、素材集及 YMG/HaiSee 取数
   均使用“主主题或 membership”口径；运行漏斗必须分开记录发现、日期/主题/质量淘汰、新增、
   复用、重复、模型失败和来源失败。
7. 每个优化主题维护由6类范式×5种窗口表述展开的30正例/30反例离线黄金集，精确率门槛85%、
   召回率门槛80%。本轮基线为后端605项通过、前端批次组件测试、类型检查和生产构建通过；
   当前环境未单独生成覆盖率报告，不得把测试通过表述为已测得覆盖率达到80%。

---

## 2026-08-17 用户工作簿来源与显式正文目标

1. `backend/scripts/import_collection_workbooks.py` 是通用、幂等的用户工作簿导入入口；站点、文章
   目标和主题定义必须来自外部工作簿及 manifest 用户数据，不得写入种子代码或连接器硬编码。
   正式导入前使用 SQLite 在线备份，现有来源按 base/home/API/discovery URL 去重并保留用户的
   合规、凭据和启停配置。
2. 未核验导入来源统一进入 fail-closed 状态：可保存目录观察结果，但不得把“工作簿标注可访问”
   当成 robots、条款或 LLM 正文许可。只有显式完成人工合规审核的来源才可自动读取正文。
3. Topic 的 `target_urls_mode` 是用户配置：`discovery` 把 URL 作为检索提示并继续采集栏目，
   `explicit` 只读取与来源精确同 origin 的挂载详情页。显式模式必须使用 `safe_fetch.py`、单页
   12 秒/目标阶段 30 秒预算，并保留排除词、日期、主题语义和质量审核，不能作为免审核旁路。
4. 工作簿标题和摘要只能作为采集线索，不得充当正式正文。原页须提取独立正文和原始发布日期；
   JSON-LD 的 `datePublished`/`dateCreated` 应在移除 script 噪声前读取。关键词与 synonyms 必须共同
   下传连接器，多语种目标不能因只使用主题主关键词而在字面预筛阶段漏采。
5. 本次导入读取10个工作簿，得到148个去重站点、63个去重详情目标；新增两个用户主题：中印贸易
